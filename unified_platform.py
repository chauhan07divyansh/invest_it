...
class UnifiedTradingPlatform:
    """Unified platform combining position and swing trading systems with robust risk management."""
    ...
    def __init__(self):
        try:
            ...
            logger.error(f"Failed to initialize UnifiedTradingPlatform: {e}")
            raise
    ...
    def set_user_preferences(self, risk_appetite, investment_horizon, capital):
        ...
        self.user_preferences = {
            ...
            'investment_horizon': investment_horizon.capitalize(),
            'capital': capital
        }
        logger.info(f"User preferences set: {self.user_preferences}")

    def create_unified_portfolio(self):
        """
        --- COMPLETELY REWRITTEN ---
        Create a blended portfolio from the best signals of both systems,
        using correct risk-based position sizing for every trade.
        """
        capital = self.user_preferences.get('capital', 100000)
        max_total_positions = 12  # Max combined positions

        print("\nScanning for all Position and Swing trading opportunities...")
        all_candidates = []

        # --- Step 1: Gather all BUY signals from both systems ---
        pos_symbols = self.position_system.get_all_stock_symbols()
        for symbol in pos_symbols:
            analysis = self.position_system.analyze_position_trading_stock(symbol)
            if analysis and analysis.get('trading_plan', {}).get('entry_signal') in ['BUY', 'STRONG BUY']:
                analysis['system_type'] = 'Position'
                all_candidates.append(analysis)

        swing_symbols = self.swing_system.get_all_stock_symbols()
        for symbol in swing_symbols:
            analysis = self.swing_system.analyze_swing_trading_stock(symbol)
            if analysis and analysis.get('trading_plan', {}).get('entry_signal') in ['BUY', 'STRONG BUY']:
                analysis['system_type'] = 'Swing'
                all_candidates.append(analysis)

        if not all_candidates:
            print(f"{Fore.RED}No suitable trading opportunities found at this time.{Style.RESET_ALL}")
            return None

        # --- Step 2: Rank all candidates by their respective scores ---
        all_candidates.sort(
            key=lambda x: x.get('position_score' if x['system_type'] == 'Position' else 'swing_score', 0),
            reverse=True
        )

        # --- Step 3: Build the final portfolio using correct risk-based sizing ---
        final_portfolio = {}
        total_allocated = 0

        print(f"\nBuilding unified portfolio from the top {len(all_candidates)} signals...")

        for candidate in all_candidates:
            if len(final_portfolio) >= max_total_positions:
                break
            ...
            try:
                ...
                if system_type == 'Position':
                    risk_per_trade_pct = self.position_system.position_trading_params['risk_per_trade']
                else:  # Swing
                    risk_per_trade_pct = self.swing_system.swing_trading_params['risk_per_trade']

                capital_at_risk_per_trade = capital * risk_per_trade_pct

                current_price = candidate.get('current_price', 0)
                stop_loss = candidate.get('trading_plan', {}).get('stop_loss', 0)

                if not all([current_price > 0, stop_loss > 0, current_price > stop_loss]):
                    continue
                ...
                if num_shares > 0 and (total_allocated + investment_amount) <= capital:
                    symbol_key = candidate.get('symbol', 'UNKNOWN').replace('.NS', '')
                    if symbol_key in final_portfolio: continue

                    final_portfolio[symbol_key] = {
                ...
                logger.error(f"Error sizing position for {candidate.get('symbol')}: {e}")
                continue

        return {'allocations': final_portfolio, 'total_allocated': total_allocated, 'capital': capital}
...
def display_unified_portfolio(portfolio_result):
    """Displays the generated unified portfolio in a clean table."""
    if not portfolio_result or not portfolio_result.get('allocations'):
        print(f"{Fore.RED}Could not generate a unified portfolio.{Style.RESET_ALL}")
        return

    portfolio = portfolio_result['allocations']
    total_allocated = portfolio_result['total_allocated']
    capital = portfolio_result['capital']

    print(f"\n{Fore.YELLOW}{'=' * 60}{Style.RESET_ALL}")
    print(f"{Fore.CYAN}{'UNIFIED PORTFOLIO RECOMMENDATION':^60}{Style.RESET_ALL}")
    print(f"{Fore.YELLOW}{'=' * 60}{Style.RESET_ALL}")

    print(f"{'Symbol':<12} {'Company':<30} {'Strategy':<10} {'Shares':<8} {'Amount (₹)':<15}")
    print("-" * 80)

    for symbol, data in portfolio.items():
        type_color = Fore.GREEN if data['system_type'] == 'Swing' else Fore.BLUE
        print(
            f"{symbol:<12} {data['company_name'][:28]:<30} "
            f"{type_color}{data['system_type']:<10}{Style.RESET_ALL} "
            f"{data['num_shares']:<8} "
            f"{data['investment_amount']:>14,.2f}"
        )

    print("-" * 80)
    print(f"Total Allocated: ₹{total_allocated:,.2f} ({total_allocated / capital:.1%})")
...
def main():
    ...
    while True:
        ...
        print("3. Create Unified Portfolio")
        print("4. Exit")
        ...
        if choice == "1":
            ...
            try:
                ...
                print(f"\n{Fore.GREEN}Please set your preferences for the unified portfolio:{Style.RESET_ALL}")
                capital = float(input("Investment Capital (INR): ").strip())

                # These preferences can be used to further refine the strategy recommendations
                risk_appetite = input("Risk Appetite (Low/Medium/High): ").strip()
                investment_horizon = input("Investment Horizon (Short/Medium/Long): ").strip()

                platform.set_user_preferences(risk_appetite, investment_horizon, capital)

                # --- THIS IS THE KEY FIX: Actually call the function and display results ---
                portfolio_result = platform.create_unified_portfolio()
                display_unified_portfolio(portfolio_result)

            except ValueError:
                print(f"{Fore.RED}Invalid capital amount. Please enter a number.{Style.RESET_ALL}")
            except Exception as e:
                logger.error(f"Error in Unified Platform mode: {e}")
